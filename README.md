...
A single static page: HTML, CSS and a bit of JavaScript — no build step, no dependencies. Navigation is two-dimensional: scrolling up and down moves between sections (Intro, Games, Sites), sideways moves through the works inside a section. Games are square tiles, the site keeps its own landscape proportion so nothing gets cropped; each accent colour is taken from the work itself. Collapses to a normal column on phones and drops the motion when the system asks for reduced motion.
